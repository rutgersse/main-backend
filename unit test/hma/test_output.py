...
from hma.output import Colors, hmaStream, debug
import hma.output
...
class TestDebug(unittest.TestCase):
    ...
    def testDebug(self):
        ...
        """
        orig_logging = hma.output.logging.debug
        s = StringIO()
        hma.output.logging.debug = s.write
        hma.output.debug_level = 0
        debug("Nothing should happen", level=1)
        self.assertEqual('', s.getvalue())
        hma.output.debug_level = 2
        debug("Something should happen", level=1)
        self.assertNotEqual('', s.getvalue())


        hma.output.logging.debug = orig_logging
...
class TestColors(unittest.TestCase):
    ...
    def testTermstyleColorsDoNotCrash(self):
        ...
        """
        c = Colors(termcolor=True)
        for func in [c.bold, c.blue, c.hma, c.red, c.yellow, c.passing,
...
class TesthmaStream(unittest.TestCase):
