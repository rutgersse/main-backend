...
from hma import djangorunner
from hma.config import mergeConfig
...
class TestDjangoRunner(unittest.TestCase):
    ...
    def test_run_testsWithLabel(self):
        ...
        dr.run_tests(('hma.test.test_version',), testing=True)
    ...
    @patch('hma.djangorunner.hmaTestSuite')
    @patch('hma.djangorunner.run')
    @patch('hma.djangorunner.loadTargets')
    def test_run_noTests(self, mock_loadTargets, mock_run, mock_hmaTestSuite):
        """
        ...
        mock_hmaTestSuite.return_value = 123

        dr.run_tests((), testing=True)

        self.assertEqual(mock_run.call_args[0][0], 123)

    @patch('hma.djangorunner.mergeConfig')
    @patch('hma.djangorunner.hmaTestSuite')
    @patch('hma.djangorunner.run')
    @patch('hma.djangorunner.loadTargets')
    def test_run_coverage(self, mock_loadTargets, mock_run, mock_hmaTestSuite, mock_mergeConfig):
        """
        If no tests are found, we create an empty test suite and run it.
        """
