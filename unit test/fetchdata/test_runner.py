...
from fetchdata.config import default_args
from fetchdata.exceptions import InitializerOrFinalizerError
from fetchdata.loader import loadTargets
from fetchdata.output import fetchdataStream
from fetchdata.runner import InitializerOrFinalizer, run
from fetchdata.suite import fetchdataTestSuite
...
class TestInitializerOrFinalizer(unittest.TestCase):
    ...
    def test_importable(self):
        ...
        global importable_function_worked
        importable_function_worked = False
        InitializerOrFinalizer('fetchdata.test.test_runner._importableFunction')()
    ...
    def test_not_callable(self):
        ...
        """
        initializer = InitializerOrFinalizer('fetchdata.test.test_runner.non_callable')
        self.assertRaises(InitializerOrFinalizerError, initializer)
    ...
    def test_crash(self):
        ...
        """
        initializer = InitializerOrFinalizer('fetchdata.test.test_runner._crashy')
        self.assertRaises(InitializerOrFinalizerError, initializer)
...
class TestRun(unittest.TestCase):
    ...
    def test_stdout(self):
        ...
        run(fetchdataTestSuite(), sys.stdout, args=self.args)
        self.assertIn('No Tests Found', self.stream.getvalue())

    def test_fetchdataStream(self):
        """
        run() can use a fetchdataStream for output.
        """
        gs = fetchdataStream(self.stream)
        run(fetchdataTestSuite(), gs, args=self.args)
        self.assertIn('No Tests Found', self.stream.getvalue())
    ...
    def test_verbose3(self):
        ...
        self.assertIn('fetchdata', self.stream.getvalue())
    ...
    def test_noTestsFound(self):
        ...
        """
        run(fetchdataTestSuite(), self.stream, self.args)
        self.assertIn('No Tests Found', self.stream.getvalue())
