...
class ConfigBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.default_filename = os.path.join(self.tmpd, ".fetchdata")
        ...
        self._write_file(self.default_filename,
                        ["# this is a test config file for fetchdata",
        ...
        self.env_filename = os.path.join(self.tmpd, "fetchdata.env")
        ...
        self.cmd_filename = os.path.join(self.tmpd, "fetchdata.cmd")
...
class TestConfig(ConfigBase):
    ...
    def test_cmd_env_nodef(self):
        """
        Setup: --config on cmd, $fetchdata_CONFIG is set, $HOME/.fetchdata does not
            exist
        Result: load --config
        """
    ...
    def test_cmd_noenv_def(self):
        """
        Setup: --config on cmd, $fetchdata_CONFIG unset, $HOME/.fetchdata exists
        Result: load --config
        """
    ...
    def test_nocmd_env_nodef(self):
        """
        Setup: no --config option, $fetchdata_CONFIG is set, $HOME/.fetchdata does not
            exist
        Result: load $fetchdata_CONFIG
        """
        os.unlink(self.cmd_filename)
    ...
    def test_nocmd_noenv_def(self):
        """
        Setup: no --config option, $fetchdata_CONFIG unset, $HOME/.fetchdata exists
        Result: load $HOME/.fetchdata
        """
        os.unlink(self.cmd_filename)
        os.unlink(self.env_filename)
        with ModifiedEnvironment(fetchdata_CONFIG=None, HOME=self.tmpd):
            ...
            ar = self.assertRaises
            ae(["fetchdata"],                  cfg.sections())
            ae(self.default_filename,      cfg.get("fetchdata", "omit-patterns"))
            ar(configparser.NoOptionError, cfg.get, "fetchdata", "run-coverage")
            ae(self.default_logging,       cfg.getboolean("fetchdata", "logging"))
            ar(configparser.NoOptionError, cfg.getboolean, "fetchdata", "no-skip-report")
            ae(self.default_version,       cfg.getboolean("fetchdata", "version"))
...
class TestMergeConfig(ConfigBase):
    ...
    def test_no_overwrite(self):
        ...
        with ModifiedEnvironment(fetchdata_CONFIG=self.env_filename, HOME=""):
